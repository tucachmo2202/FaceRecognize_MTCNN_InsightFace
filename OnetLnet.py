...
try:
    ...
    izip =zip
from Utils.helper import nms, adjust_input, generate_bbox, detect_first_stage_warpper


class OnetLnetAlignment(object):
    """
    ...
    def detect_landmark(self, img, bboxes):
        ...
        for i in range(num_box):
            ...
            #   print tmph[i],"  ",tmpw[i],"  "
            #   print y[i],"   ",ey[i],"   ", x[i],"   ",ex[i]
            tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1, :] = img[y[i]:ey[i] + 1, x[i]:ex[i] + 1, :]
            input_buf[i, :, :, :] = adjust_input(cv2.resize(tmp, (48, 48)))
        if input_buf.shape[0]==0:
            return None
        output = self.ONet.predict(input_buf)
        # filter the total_boxes with threshold]
        passed = np.where(output[2][:, 1] > self.threshold[2])
        passed_ = passed[0]
        total_boxes = total_boxes[[i for i in passed_]]
        ...
        # print ("The third stage is : ", time.time() - t)
        # t = time.time()
        #############################################
        # extended stage
        #############################################
        ...
        for i in range(5):
            ...
            for j in range(num_box):
                ...
                # print y[i],"  ",ey[j],"   ",x[j],"   ",ex[j]
                # print dy[j]," ",edy[j],"  ",dx[j],"   ",edx[j]
        ...
        # print ("The final stage is : ", time.time() - t)
        return total_boxes, points

    def extract_image_chips(self, img, points, desired_size=256, padding=20):
        """
            crop and align face
        Parameters:
        ----------
